export default class Decimal {
    ...
    constructor(v: string | number | bigint | Decimal) {
      if (v instanceof Decimal) { this.n = v.n; return; }
      if (typeof v === 'bigint') { this.n = v * Decimal.TEN_S; return; }
      if (typeof v === 'number') v = Number.isFinite(v) ? v.toString() : '0';
  
      const s = (v as string).trim();
      if (!s) { this.n = 0n; return; }
    ...
    // 比較等
    eq(b: Decimal): boolean { return this.n === b.n; }
    lt(b: number | string | Decimal): boolean {
      const x = Decimal.coerce(b);
      return this.n < x.n;
    }
    isZero(): boolean { return this.n === 0n; }
    isFinite(): boolean { return true; } // BigInt なので常に有限
    ...
    toFixed(dp: number, _mode: number): string {
      ...
      return dp > 0 ? `${sign}${intPart.toString()}.${frac}` : `${sign}${intPart.toString()}`;
    }
    ...
    private static pow10(k: number): bigint { let p = 1n; for (let i=0;i<k;i++) p *= 10n; return p; }
